...
// Import the hyperchat tests
use crate::hyperchat_ops::run_hyperchat_tests;

pub fn run_client_ops(log_file: &mut File, client_addresses: &Vec<Address>) -> anyhow::Result<()> {
    // Run basic client operations (API tests)
    ...
    // Run specific Hyperchat tests
    write_log(log_file, "Starting Hyperchat application tests")?;
    match run_hyperchat_tests(log_file, client_addresses) {
        Ok(_) => write_log(log_file, "Hyperchat tests completed successfully")?,
        Err(e) => write_log(log_file, &format!("Hyperchat tests failed: {:?}", e))?,
    }
    
    write_log(log_file, "All tests completed")?;
    Ok(())
}
